function [y,Jyl,Jyr,ilocl,ilocr,evalcnt,ttf_visitedMap]=exp_greedy2_cross_interp(n, fun,fval, tol,preevalcnt,delta,varargin)
% Compute tensor train approximation to exp(-fun/delta)
...
ry = ones(d+1,1);
y = cell(d,1);
fun = @(zz)fun(zz)./min(fval(0.2.*ones(1,d)+0.01*randn(1,d)),1); % for better stablility to aovid rounding off-error in crossing approximation
fun = @(zz)exp(-fun(zz)./delta);
...
for i=d:-1:1
    ...
    [cry1, evalcnt, ttf_visitedMap] = autovecfun(fun, J, evalcnt, ttf_visitedMap); %value of f at J indices
    if (~isempty(aux))     
        craux = zeros(ry(i)*n(i)*ry(i+1), Raux);
        for j=1:Raux
            craux1 = reshape(aux{i,j}, raux(i,j), n(i)*raux(i+1,j));
            craux1 = phiauxl{i,j}*craux1; % 1 if no auxilary function
            craux1 = reshape(craux1, ry(i)*n(i), raux(i+1,j));
    ...
    if (i>1)        
        cry1 = reshape(cry1, ry(i), n(i)*ry(i+1)); %value of f at J indices
        [v,ilocr{i}] = max(abs(cry1.')); % find the element with maximum volume and take this guy as pivot
        ilocr{i} = ilocr{i}(:);
        mid_inv{i,1} = 1/cry1(:,ilocr{i}); % we have 1/maximum element which causes errors.
        mid_inv{i,2} = 1;
        mid_inv{i,1} = avoid_downflow(mid_inv{i,1});
...
for i=1:d
    J = indexmerge(Jyl{i}, (1:n(i))', Jyr{i+1});
    [cry1, evalcnt, ttf_visitedMap] = autovecfun(fun, J, evalcnt, ttf_visitedMap);
    if (~isempty(aux))
        craux = zeros(ry(i)*n(i)*ry(i+1), Raux);
        for j=1:Raux
            craux1 = reshape(aux{i,j}, raux(i,j), n(i)*raux(i+1,j));
    ...
    if (i<d)
        ...
        mid_inv{i+1,2} = 1;     
        mid_inv{i+1,1} = avoid_downflow(mid_inv{i+1,1});
        Jyl{i+1} = indexmerge(Jyl{i}, (1:n(i))');
        Jyl{i+1} = Jyl{i+1}(ilocl{i+1},:);
        if (~isempty(aux))
            for j=1:Raux
...
%% Continue to do sweep until stop
last_sweep = false;
maxy = 0;
max_dx = 0;
swp = 1;
...
while (swp<=nswp)
    ...
    if (testsz>0)
        if (strcmp(locsearch, 'als'))
            ...
            J = indexmerge(J1c,Jr);
            [cre1, evalcnt, ttf_visitedMap] = autovecfun(fun, J, evalcnt, ttf_visitedMap);
            if (~isempty(aux))
            ...
            J = indexmerge(Jl,J2c);
            [cre2, evalcnt, ttf_visitedMap] = autovecfun(fun, J, evalcnt, ttf_visitedMap);
            ...
            [crt, evalcnt, ttf_visitedMap] = autovecfun(fun, J, evalcnt, ttf_visitedMap);
        ...
        if (dx>tol)
            ...
            [cre1, evalcnt, ttf_visitedMap] = autovecfun(fun, Jl, evalcnt, ttf_visitedMap);
            ...
            [cre2, evalcnt, ttf_visitedMap] = autovecfun(fun, Jr, evalcnt, ttf_visitedMap);
            ...
            mid_inv{i+1,1} = avoid_downflow(mid_inv{i+1,1});
            mid_inv{i+1,2} = avoid_downflow(mid_inv{i+1,2});
...
function [y,evalcnt,ttf_visitedMap]=autovecfun(fun, J,evalcnt,ttf_visitedMap)
    sz = size(J,1);
    y = zeros(sz, 1);
    if ttf_visitedMap == 0
        for j=1:sz
        ...
        cntj = zeros(sz,1);
        for j=1:sz
            if isKey(ttf_visitedMap, mat2str(J(j,:)))
                y(j) = ttf_visitedMap(mat2str(J(j,:)));
            else
                y(j) = fun(J(j,:));
                markAsVisited(ttf_visitedMap, J(j,:), y(j));
                cntj(j) = 1;
            end
             markAsVisited(ttf_visitedMap, J(j,:), y(j));
        end
            evalcnt = evalcnt + sum(cntj);
    end
    evalcnt = evalcnt+ sz;
end


% % Function to mark an entry as visited
function markAsVisited(map, indexTuple, value)
    % Convert the index tuple to a string key
    key = mat2str(indexTuple);
