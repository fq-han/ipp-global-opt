function [x,fval,exitFlag,output,points] = my_particleswarm(fun, nvars, lb, ub, userOptions)
...
%
%   [X,FVAL,EXITFLAG,OUTPUT,POINTS] = PARTICLESWARM(FUN, ...) also returns
...
if options.UseAsync
    ...
    state = updateParticles(state,adaptiveInertia,bestNeighborIndex,cSelf,cSocial,1:numParticles,numParticles,nvars,lbMatrix,ubMatrix);
...
function newVelocities = updateVelocities(state,adaptiveInertia,bestNeighborIndex,cSelf,cSocial,pIdx,nvars)
...
function [newPositions,tfInvalid] = updatePositions(state,lbMatrix,ubMatrix,pIdx,numParticles,nvars)
...
function state = updateParticles(state,adaptiveInertia,bestNeighborIndex,cSelf,cSocial,pIdx,numParticles,nvars,lbMatrix,ubMatrix)

% Update the velocities.
state.Velocities(pIdx,:) = updateVelocities(state,adaptiveInertia,bestNeighborIndex,cSelf,cSocial,pIdx,nvars);

% Update the positions.
[state.Positions(pIdx,:),tfInvalid] = updatePositions(state,lbMatrix,ubMatrix,pIdx,numParticles,nvars);
