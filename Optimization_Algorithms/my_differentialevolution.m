function varargout = my_differentialevolution(varargin)
...
xkerror = zeros(DEParams.maxiter,1);
...
while k <= size(paramDefCell, 1)
  ...
  if parameterDim == 1
    ...
    parameterDimVector(k : k + parameterDim - 1, 1) = parameterDim; 
...
xk_hist = zeros(DEParams.maxiter,D);
...
while ...
  ...
  iterationNr = iterationNr + 1;
  xkerror(iterationNr) = my_error_opt(xex,bestmem);
  xk_hist(iterationNr,:) = bestmem;
  if xkerror(iterationNr) < term_tol
       varargout = {bestmem', bestval, bestFctParams, iterationNr, nfeval.overall, xkerror, xk_hist,timeOverFileName};
       return;
  end
